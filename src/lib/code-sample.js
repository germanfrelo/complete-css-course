...
class CodeSample {
	...
	get items() {
		...
		const getCodeSampleName = (codeSamplePath) => {
			const pathParts = codeSamplePath
		...
		const buildCodeSample = (
			...
			if (
				fs.existsSync(path.resolve(codeSamplePath, `${codeSampleName}.njk`))
			) {
			...
			// If not, we look for a data file
				...
				fs.existsSync(path.resolve(codeSamplePath, `${codeSampleName}.json`))
			) {
		...
		codeSamples.forEach((item) => {
			...
			const codeSampleResponse = buildCodeSample(
				codeSampleRoot,
				codeSampleName,
			);
			...
			if (codeSampleResponse.data.sort !== "source") {
				if (codeSampleResponse.variants) {
					codeSampleResponse.variants = codeSampleResponse.variants.sort(
	...
	renderPattern(path, data = {}) {
		...
		if (isVariant) {
			pattern = primaryPattern.variants.find(
				(x) => x.name === path.split("/")[1],
			);
		}
